...
WORKSPACE_NAME = "rootweiler"  # change here if your workspace slug differs
WORKFLOW_ID = "leafy"          # change here if your workflow slug differs
API_URL = "https://serverless.roboflow.com"  # or "https://detect.roboflow.com" if needed
...
def _call_roboflow_workflow_raw(image_bytes: bytes) -> Optional[Any]:
    ...
    - Calls run_workflow (preferred) or infer_from_workflow (fallback)

    Returns the raw `result` object from the SDK, or None on failure.
    """
    ...
    try:
        # Preferred: modern inference-sdk with run_workflow
        try:
            result = client.run_workflow(
                workspace_name=WORKSPACE_NAME,
                workflow_id=WORKFLOW_ID,
                images={"image": tmp_path},
                # NOTE: DO NOT pass use_cache here – your SDK version doesn't accept it
            )
        except AttributeError:
            # Older inference-sdk: infer_from_workflow
            try:
...
def _run_roboflow_workflow(image_bytes: bytes) -> Optional[Dict[str, object]]:
    ...
    if "output2" in item:
        preds = item["output2"]
    elif "model_predictions" in item:
